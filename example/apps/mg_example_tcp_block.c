...
static void prvThreadEntry(void *param)
{
    ...
    /*get tcp state*/
    Enum_SocketState nTcpState = CLOSED;
    iResult = MG_SOCK_GetSocketState(fd, &nTcpState);
    APP_DEBUG("fd %d tcp state %d line[%d] iResult %d", fd, nTcpState, __LINE__, iResult);
    ...
    /*send*/
    s32 nWrittenLen = 0;
    nWrittenLen = MG_SOCK_Send(fd, (const u8 *)SEND_MSG, (u32)MG_UTILS_Strlen(SEND_MSG));
    APP_DEBUG("TCP SEND line[%d] nWrittenLen:%d", __LINE__, nWrittenLen);

    /*recv*/
    if(nWrittenLen > 0)
    {
        s32 nRecvLen = MG_SOCK_Recv(fd, RecvBuff, RECV_BUFF_SIZE - RECV_BUFF_SIZE_OFSSET);
        if(nRecvLen > 0)
        {
