...
#define __CURRENT_FILE_NAME__ app
#define __EC618 1
#define CHIP_EC618 1
#define CORE_IS_AP 1
...
_Bool npiGetProdModeEnableStatus(void);
void npiSetProdModeEnableStatus(_Bool enable);
#define SDK_MAJOR_VERSION "001"
#define SDK_MINOR_VERSION "000"
#define SDK_RA_VERSION "xxx"
#define SDK_PATCH_VERSION "pb6.011"
...
static void showBootInfo(void)
{
    ...
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_MCI__showBootInfo_mci_1, P_INFO, pVerBuf); {(void)"version:%s";} }while(0);
 do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_MCI__showBootInfo_mci_2, P_INFO, apRstState, cpRstState); {(void)"boot cause: AP:%d, CP:%d";} }while(0);
}
...
osSemaphoreId_t simDetectSem = ((void *)0);
...
static void UserAppTask(void *arg)
{
    ...
    simDetectSem = osSemaphoreNew(3, 0, ((void *)0));
    osDelay(((UINT32)(((500))*((TickType_t)1000))/1000));
    showBootInfo();
    ...
    while (1)
    {
        if (osOK == osSemaphoreAcquire(simDetectSem, 0xFFFFFFFFU))
        {
            if (MEIG_Util_SimHotplugIsEnable())
            {
                ...
                if (bSimPlugIn)
                    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PLA_APP__ApUserAppTask_0, P_VALUE); {(void)"SIM Plug In";} }while(0);
                else
                    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PLA_APP__ApUserAppTask_1, P_VALUE); {(void)"SIM Plug Out";} }while(0);
            }
        }
        osDelay(((UINT32)(((200))*((TickType_t)1000))/1000));
    }
}
...
{
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PLA_APP__EC_CHIP_VERSION_1, P_INFO, "EcChipVerEc618CoreAp"); {(void)"%s";} }while(0);
...
void main_entry(void)
{
    BSP_CommonInit();
    osKernelInitialize();
    registerAppEntry(appInit, ((void *)0));
