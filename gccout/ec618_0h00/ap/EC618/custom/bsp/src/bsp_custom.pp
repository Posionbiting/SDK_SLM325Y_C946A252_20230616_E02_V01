...
#define __CURRENT_FILE_NAME__ bsp_custom
#define __EC618 1
#define CHIP_EC618 1
#define CORE_IS_AP 1
...
typedef enum
{
    TIMER_INTERNAL_CLOCK = 0U,
    TIMER_EXTERNAL_CLOCK = 1U,
} TimerClockSource_t;
typedef enum
{
    TIMER_MATCH0_SELECT = 0U,
    TIMER_MATCH1_SELECT = 1U,
    TIMER_MATCH2_SELECT = 2U,
} TimerMatchSelect_t;
typedef enum
{
    TIMER_RELOAD_DISABLE = 0U,
    TIMER_RELOAD_ON_MATCH0 = 1U,
    TIMER_RELOAD_ON_MATCH1 = 2U,
    TIMER_RELOAD_ON_MATCH2 = 3U,
} TimerReloadOption_t;
typedef struct
{
    uint32_t pwmFreq_HZ;
...
typedef struct
{
    ...
    TimerReloadOption_t reloadOption;
    uint32_t initValue;
    uint32_t match0;
    uint32_t match1;
    uint32_t match2;
...
typedef enum
{
    TIMER_INTERRUPT_DISABLED = 0U,
    TIMER_INTERRUPT_LEVEL = 1U,
    TIMER_INTERRUPT_PULSE = 2U,
} TimerInterruptConfig_t;
typedef enum
{
    TIMER_MATCH0_INTERRUPT_ENABLE = (0x1UL << (3)),
    TIMER_MATCH1_INTERRUPT_ENABLE = (0x1UL << (4)),
    TIMER_MATCH2_INTERRUPT_ENABLE = (0x1UL << (4)),
} TimerInterruptEnable_t;
typedef enum
{
    TIMER_MATCH0_INTERRUPT_FLAG = (0x1UL << (0)),
    TIMER_MATCH1_INTERRUPT_FLAG = (0x1UL << (1)),
    TIMER_MATCH2_INTERRUPT_FLAG = (0x1UL << (2)),
...
void ResetLockupCfg(_Bool apLockupRstEn, _Bool cpLockupRstEn);
void ResetApplyAPLockupCfg(void);
void ResetECSystemReset(void);
void ResetStartPorReset(ResetReason_e reason);
_Bool ResetIsSwPorReset(ResetReason_e *pReason);
...
#define CCIO_DEV_ID_SET(dt,st,llsn,no) ((((dt) << 11) & CCIO_DEV_TYPE_MASK) | (((st) << 9) & CCIO_DEV_SUBTYPE_MASK) | (((llsn) << 5) & CCIO_DEV_LLSN_MASK) | ((no) & CCIO_DEV_NO_MASK))
...
typedef enum
{
    ...
    CCIO_DEV_HWACM_UL = (1 << 0),
    CCIO_DEV_HWACM_DL = (1 << 1),
...
typedef enum
{
    CCIO_TASK_FLAG_NONE = 0x0,
    CCIO_TASK_FLAG_TX = 0x1,
    CCIO_TASK_FLAG_RX = 0x2,
}CcioTaskOperFlag_e;
typedef enum
{
...
#define CCIO_CHAN_MSG_CF_COS0 0x00000000
#define CCIO_CHAN_MSG_CF_COS1 0x00010000
#define CCIO_CHAN_MSG_CF_COS2 0x00020000
#define CCIO_CHAN_MSG_CF_DIN 0x00400000
#define CCIO_CHAN_MSG_CF_IRQ 0x00800000
...
#define CCIO_CHAN_MSG_ID_BUILD(id,isIrq,isDin,cos) ((id) | ((isIrq) ? CCIO_CHAN_MSG_CF_IRQ : 0) | ((isDin) ? CCIO_CHAN_MSG_CF_DIN : 0) | ((cos) & 0x000F0000))
#define CCIO_CHAN_MSG_BUILD(chmsg,id,chdev,flgs,dat,xtr) do { ((CcioChanMessage_t*)(chmsg))->msgId = id; ((CcioChanMessage_t*)(chmsg))->lanDev = (CcioDevice_t*)(chdev); ((CcioChanMessage_t*)(chmsg))->flags = (flgs); ((CcioChanMessage_t*)(chmsg))->info.data = (uint32_t)(dat); ((CcioChanMessage_t*)(chmsg))->extras.data = (uint32_t)(xtr); }while(0)
...
typedef struct CcioDevice
{
    ...
    uint32_t wkState :2;
    uint32_t bmHwAcm :2;
    uint32_t isWaitTxCmplt :1;
    uint32_t asgnTxCos :2;
    uint32_t rsvdBits :25;
    void *chent;
    chdevOutFunc chdevOutFn;
    chdevInCallback chdevInCb;
    chdevExecFunc chdevExecFn;
    chdevStatusCallback chdevStatusCb;
    void *txHead;
    void *txTail;
    void *extras;
}CcioDevice_t;
...
typedef struct CcioEntity
{
    ...
    void *chdev;
    void *rbuf;
    chentOutFunc chentOutFn;
    chentInCallback chentInCb;
    chentStatusCallback chentStatusCb;
}CcioEntity_t;
typedef struct
{
    CcioEntity_t base;
...
typedef struct
{
    ...
    uint32_t ctrlSetting;
    uint32_t baudRate;
}UartHwConf_t;
typedef struct
{
    UartHwConf_t hwConf;
    ARM_DRIVER_USART *drvHandler;
    uint8_t mainUsage;
    uint8_t speedType;
    uint8_t bmCreateFlag;
    uint8_t isDftAtPort :1;
    uint8_t rsvd :7;
}UartDevConf_t;
...
void chargeDetectInit(chargeStatusCb cb, _Bool monitorEn, uint32_t sample_dly);
void chargeDetectDeinit(void);
#define _WDT_EC618_H 
typedef enum
{
    WDT_INTERRUPT_ONLY_MODE = 0U,
    WDT_INTERRUPT_RESET_MODE = 1U,
} WdtMode_e;
typedef struct
{
    WdtMode_e mode;
...
_Bool WDT_getStartStatus(void);
#define WDT_TIMEOUT_VALUE (20)
#define HAL_PWRKEY_H 
#define PWR_KEY_H 
...
void pwrKeyHwInit(_Bool pullUpEn);
void pwrKeyHwDeinit(_Bool pullUpEn);
_Bool pwrKeyGetPinLevel(void);
pwrKeyWorkMode pwrKeyGetPwrKeyMode(void);
void pwrKeyIntHandler(void);
void pwrKeyInit(pwrKeyWorkMode workMode, _Bool pullUpEn, pwrKeyDly_t dlyCfg, pwrKeyCallback_t Callback);
void pwrKeyDeinit(_Bool pullUpEn);
#define _HAL_MISC_H 
...
void delay_us(uint32_t us);
void apmuBootDbgGPIOSet(_Bool level);
_Bool apmuGetLongSlpCfg(void);
...
typedef struct
{
    ...
    uint8_t rsvd : 6;
    alarmVoltThd voltThd;
    alarmThmThd thermThd;
    alarmHysteresisRange hysterThd;
}alarmParamCfg_t;
typedef void(* vbatLowCallback_t)(_Bool isVoltAboveThd);
typedef void(* thmHighCallback_t)(_Bool isTempAboveThd);
...
#define DB_VERSION_UNIQ_ID "0x37e834b8"
...
__attribute__((used)) static uint8_t rsvUsrRam[1];
typedef int (*pfn_PadWakeupHook)(uint32_t pad_num);
extern BOOL excepIsInExcephandler(void);
extern void apmuSetCPFastBoot(_Bool force_on);
...
void InitAtUartHandler(uint8_t uartIndex, uint32_t baudRate, uint32_t frameFormat)
{
    ...
    uartHwConf->ctrlSetting = (0x01UL << 0);
    uartHwConf->baudRate = baudRate;
    uartFrameFmt.wholeValue = frameFormat;
    switch(uartFrameFmt.config.dataBits)
    {
        case 1:
            uartHwConf->ctrlSetting |= (7UL << 8);
            break;
        default:
            uartHwConf->ctrlSetting |= (0UL << 8);
            break;
    }
    switch(uartFrameFmt.config.parity)
    {
        case 1:
            uartHwConf->ctrlSetting |= (2UL << 12);
            break;
        case 2:
            uartHwConf->ctrlSetting |= (1UL << 12);
            break;
        default:
            uartHwConf->ctrlSetting |= (0UL << 12);
            break;
    }
    switch(uartFrameFmt.config.stopBits)
    {
        case 2:
            uartHwConf->ctrlSetting |= (1UL << 14);
            break;
        default:
            uartHwConf->ctrlSetting |= (0UL << 14);
            break;
    }
    switch(uartFrameFmt.config.flowControl)
    {
        case 3:
            uartHwConf->ctrlSetting |= (3UL << 16);
            break;
        case 2:
            uartHwConf->ctrlSetting |= (2UL << 16);
            break;
        case 1:
            uartHwConf->ctrlSetting |= (1UL << 16);
            break;
        case 0:
        default:
            uartHwConf->ctrlSetting |= (0UL << 16);
            break;
    }
    switch(baudRate)
    {
    ...
    do { { if((uartDevConf.drvHandler) == 0) { ec_assert_regs(); excepEcAssert(__FUNCTION__, 279, (uint32_t)(uartDevConf.drvHandler),(uint32_t)(0),(uint32_t)(0)); while(1); } } } while(0);
...
{
    slpManAonWdtStop();
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PLA_APP__enter_NMI_handler, P_ERROR); {(void)"WDT timeout!!! Enter NMI Handler!!!";} }while(0);
    ...
    else
    {
        ...
        {
            do { { if((0) == 0) { ec_assert_regs(); excepEcAssert(__FUNCTION__, 421, (uint32_t)(0),(uint32_t)(0),(uint32_t)(0)); while(1); } } } while(0);
        }
    }
}
...
static int32_t UartPwmSetup(uint32_t pwmInst, uint32_t clkFreq, uint16_t dutyCyclePct, uint32_t cycleTimeMs)
{
    ...
    pwmConfig.srcClock_HZ = clkFreq * clkMagnif;
...
{
    do { { if((uartIdx < PORT_USART_MAX && gUartDtrInst[uartIdx].hasInit) == 0) { ec_assert_regs(); excepEcAssert(__FUNCTION__, 543, (uint32_t)(uartIdx),(uint32_t)(gUartDtrInst[uartIdx].hasInit),(uint32_t)(0)); while(1); } } } while(0);
    ...
    if (GPIO_getInterruptFlags(gUartDtrInst[uartIdx].instId) & (1 << gUartDtrInst[uartIdx].pinId))
    {
        ...
        {
            do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PLA_APP__UART_DTR_ISR, P_WARNING, uartIdx); {(void)"uart(%d): DTR GPIO is not inited!!!";} }while(0);
        }
...
static int32_t UartRiPwmInit(uint8_t uartIdx, uint16_t dutyCyclePct, uint32_t cycleTimeMs)
{
    ...
    switch(uartIdx)
    {
        case PORT_USART_1:
        {
            padIndex = (44);
            pwmInst = (1);
            pwmClkId = (FCLK_TIMER1);
            pwmClkSel = (FCLK_TIMER1_SEL_26M);
            break;
        }
        case PORT_USART_2:
        {
            padIndex = (43);
            pwmInst = (0);
            pwmClkId = (FCLK_TIMER0);
            pwmClkSel = (FCLK_TIMER0_SEL_26M);
            break;
        }
        default:
            return -1;
...
void WakeupPadInit(void)
{
    ...
    wakeupPadSetting.negEdgeEn = 1;
    wakeupPadSetting.posEdgeEn = 1;
    wakeupPadSetting.pullDownEn = 0;
    wakeupPadSetting.pullUpEn = 1;
    apmuSetWakeupPadCfg(WAKEUP_PAD_0, 1, &wakeupPadSetting);
    apmuSetWakeupPadCfg(WAKEUP_PAD_1, 1, &wakeupPadSetting);
    apmuSetWakeupPadCfg(WAKEUP_PAD_2, 1, &wakeupPadSetting);
    apmuSetWakeupPadCfg(WAKEUP_PAD_3, 1, &wakeupPadSetting);
    apmuSetWakeupPadCfg(WAKEUP_PAD_5, 1, &wakeupPadSetting);
    __NVIC_EnableIRQ(PadWakeup0_IRQn);
    __NVIC_EnableIRQ(PadWakeup1_IRQn);
    __NVIC_EnableIRQ(PadWakeup2_IRQn);
    __NVIC_EnableIRQ(PadWakeup3_IRQn);
    __NVIC_EnableIRQ(PadWakeup5_IRQn);
    wakeupPadSetting.pullUpEn = 0;
    wakeupPadSetting.pullDownEn = 0;
    apmuSetWakeupPadCfg(WAKEUP_PAD_4, 0, &wakeupPadSetting);
    __NVIC_DisableIRQ(PadWakeup4_IRQn);
    srvWakeupInit();
}
void reConfig(void)
{
    ...
    if (mg_nvmConfig.bSimVoltTrigMode == 0)
    {
        wakeupPadSetting.pullDownEn = 0;
        wakeupPadSetting.pullUpEn = 1;
        apmuSetWakeupPadCfg(WAKEUP_PAD_2, 1, &wakeupPadSetting);
...
{
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__chargeStatusCb_1, P_VALUE, status); {(void)"Charge Status update = %d";} }while(0);
}
void powerKeyStatusUpdate(pwrKeyPressStatus status)
{
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__powerKeyStatusUpdate_1, P_VALUE, status); {(void)"PowerKey Status update to = %d";} }while(0);
    ...
    {
        do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__powerKeyStatusUpdate_2, P_WARNING); {(void)"PowerKey Start Power Off";} }while(0);
        uniLogFlushOut();
        pwrKeyStartPowerOff();
    }
}
...
{
    if(BSP_GetPlatConfigItemValue(PLAT_CONFIG_ITEM_PWRKEY_MODE) == 2)
    {
        pwrKeyDeinit(0);
    }
    else
    {
        ...
        pwrKeyInit(PWRKEY_WAKEUP_LOWACTIVE_MODE, 1, pwrKeyDlyCfg, powerKeyStatusUpdate);
    }
}
...
void BSP_UsbDeInit(void)
{
    ...
    ret = usbstack_deinit();
    do { { if(((ret==0)) == 0) { ec_assert_regs(); excepEcAssert(__FUNCTION__, 1009, (uint32_t)(usbd_get_mod_last_err()),(uint32_t)(0),(uint32_t)(0)); while(1); } } } while(0);
}
uint32_t BSP_UsbGetVBUSMode(void)
{
...
void BSP_UsbInit(void)
{
    ...
    {
        do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__usblpw_wkup_or_init_pre_0, P_WARNING, usblpw_rd_reg_retwkup_ctxstat()); {(void)"ctx stat reg %d";} }while(0);
        if (usblpw_is_retwkup_support())
        {
            usblpw_get_pwr_info(&usb_pwr_state, &usb_retwkup_bootstat, &usb_lastwkup_type);
            do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__usblpw_wkup_or_init_0, P_VALUE, (uint32_t)usb_pwr_state, (uint32_t)usb_retwkup_bootstat, (uint32_t)usb_lastwkup_type, usblpw_retothwk_get_cur_stg()); {(void)"pwr state 0x%x, bootstat 0x%x, lastwkup 0x%x, othwk stg 0x%x ";} }while(0);
            if(usblpw_is_retwkups2_start())
            {
                ...
                do { { if(((ret==0)) == 0) { ec_assert_regs(); excepEcAssert(__FUNCTION__, 1075, (uint32_t)(0),(uint32_t)(0),(uint32_t)(0)); while(1); } } } while(0);
            }
            else if (usblpw_is_retothwks2_start())
            {
                ...
                usbinit_mode = 0x14;
                do { { if(((ret==0)) == 0) { ec_assert_regs(); excepEcAssert(__FUNCTION__, 1089, (uint32_t)(usbd_get_mod_last_err()),(uint32_t)(usblpw_retothwk_get_cur_stg()),(uint32_t)(usblpw_get_retothwk_proc_stat())); while(1); } } } while(0);
            }
        }
        if(usbinit_mode ==0)
        {
            ret = usbstack_init();
            usbinit_mode = 0x10;
            do { { if(((ret==0)) == 0) { ec_assert_regs(); excepEcAssert(__FUNCTION__, 1185, (uint32_t)(usbd_get_mod_last_err()),(uint32_t)(0),(uint32_t)(0)); while(1); } } } while(0);
        }
    }
    if (usblpw_is_retwkup_support())
    {
        ...
        do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__usblpw_wkup_or_init_1, P_VALUE, (uint32_t)usb_pwr_state, (uint32_t)usb_retwkup_bootstat, (uint32_t)usb_lastwkup_type, (uint32_t)usbinit_mode); {(void)"pwr state 0x%x, bootstat 0x%x, lastwkup 0x%x, inimod 0x%x ";} }while(0);
        if (usbstack_get_ctx_vbus_mode()==0)
        {
            do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__usblpw_wkup_or_init_2, P_VALUE, usblpw_retothwk_get_cur_stg(), usblpw_get_retothwk_proc_stat()); {(void)"othwk stg 0x%x, proc_stat 0x%x";} }while(0);
        }
    }
    endTick = xTaskGetTickCount();
    if(usbinit_mode==1)
    {
        do { { if(((endTick-startTick) < 30) == 0) { ec_assert_regs(); excepEcAssert(__FUNCTION__, 1208, (uint32_t)(startTick),(uint32_t)(endTick),(uint32_t)(0)); while(1); } } } while(0);
    }
}
slpManSlpState_t CheckUsrdefSlpStatus(void)
{
    ...
    {
        if((slpManGetWakeupPinValue() & (0x1<<2)) == (0x1<<2))
        {
...
void BSP_CustomInit(void)
{
    ...
    {
        SetUnilogUart(PORT_USART_0, BSP_GetPlatConfigItemValue(PLAT_CONFIG_ITEM_LOG_BAUDRATE), 1);
        uniLogInitStart(UART_0_FOR_UNILOG);
    }
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PLA_STRING__build_info, P_SIG, getBuildInfo()); {(void)"%s";} }while(0);
    if(apmuGetAPLLBootFlag()==AP_BOOT_FROM_POWER_ON)
    {
        extern void EFuseGetDevVer(uint8_t *DevVer);
        EFuseGetDevVer(&DevVer);
        do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PLA_STRING__build_info_1, P_SIG, DevVer); {(void)"fuse dev 0x%x";} }while(0);
    }
...
{
    if(slpManExtIntPreProcess(PadWakeup0_IRQn)==0)
        return;
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__Pad0_WakeupIntHandler_1, P_VALUE); {(void)"Pad0 Wakeup";} }while(0);
    uniLogFlushOut();
    wakeup_in_process(0);
}
pfn_PadWakeupHook p_funcPadWakeupHook = ((void *)0);
...
{
    if(slpManExtIntPreProcess(PadWakeup1_IRQn)==0)
        return;
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__Pad1_WakeupIntHandler_1, P_VALUE); {(void)"Pad1 Wakeup";} }while(0);
    uniLogFlushOut();
    if (p_funcPadWakeupHook)
    {
...
BOOL BSP_SimGetSimStatus(void)
{
    ...
    MG_NV_GetMgSysParam(&mg_nvmConfig);
    if(mg_nvmConfig.bSimVoltTrigMode == 0)
        bSimPlugIn = (mg_nvmConfig.bSimVoltTrigMode == level) ? 1 : 0;
    else
        bSimPlugIn = ((mg_nvmConfig.bSimVoltTrigMode << 2) == level) ? 1 : 0;
    return bSimPlugIn;
}
void Pad2_WakeupIntHandler(void)
{
    static uint8_t status = 0xFF;
    if(slpManExtIntPreProcess(PadWakeup2_IRQn)==0)
        return;
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__Pad2_WakeupIntHandler_1, P_VALUE); {(void)"SIM Detect interrupt occurred";} }while(0);
    uniLogFlushOut();
...
{
    if(slpManExtIntPreProcess(PadWakeup3_IRQn)==0)
        return;
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__Pad3_WakeupIntHandler_1, P_VALUE); {(void)"Pad3 Wakeup";} }while(0);
    uniLogFlushOut();
    wakeup_in_process(3);
}
void Pad4_WakeupIntHandler(void)
{
    if(slpManExtIntPreProcess(PadWakeup4_IRQn)==0)
        return;
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__Pad4_WakeupIntHandler_1, P_VALUE); {(void)"Pad4 Wakeup";} }while(0);
    uniLogFlushOut();
    wakeup_in_process(4);
}
void Pad5_WakeupIntHandler(void)
{
    if(slpManExtIntPreProcess(PadWakeup5_IRQn)==0)
        return;
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__Pad5_WakeupIntHandler_1, P_VALUE); {(void)"Pad5 Wakeup";} }while(0);
    uniLogFlushOut();
    if (p_funcPadWakeupHook)
    {
...
{
    if(slpManExtIntPreProcess(PwrkeyWakeup_IRQn)==0)
        return;
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__PwrKey_WakeupIntHandler_1, P_VALUE); {(void)"Pwr key Wakeup";} }while(0);
    uniLogFlushOut();
    pwrKeyIntHandler();
}
void ChrgPad_WakeupIntHandler(void)
{
    if(slpManExtIntPreProcess(ChrgpadWakeup_IRQn)==0)
        return;
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PMU__ChrgPad_WakeupIntHandler_1, P_VALUE); {(void)"charge Wakeup";} }while(0);
    uniLogFlushOut();
    chargeIntHandler();
}
void Usim1GpioConfig(BOOL bSetUsimFunc)
{
    ...
    if (bSetUsimFunc == ((BOOL)1))
    {
        PAD_getDefaultConfig(&padConfig);
        padConfig.mux = PAD_MUX_ALT4;
        PAD_setPinConfig(19, &padConfig);
        PAD_getDefaultConfig(&padConfig);
        padConfig.mux = PAD_MUX_ALT4;
        PAD_setPinConfig(20, &padConfig);
        PAD_getDefaultConfig(&padConfig);
        padConfig.mux = PAD_MUX_ALT4;
        PAD_setPinConfig(21, &padConfig);
    }
    else
    {
...
void srvWakeupInit(void)
{
    ...
    *(uint32_t *)0x4D020170 = 0x01;
    delay_us(1000);
    apmuSetWakeupPadCfg(WAKEUP_PAD_5, 0, ((void *)0));
    __NVIC_DisableIRQ(PadWakeup5_IRQn);
    PAD_getDefaultConfig(&padConfig);
    ...
    PAD_setPinConfig(42, &padConfig);
    GpioPinConfig_t PinConfig;
    PinConfig.pinDirection = GPIO_DIRECTION_OUTPUT;
    PinConfig.misc.initOutput = 1;
    GPIO_pinConfig(22 / 16, 22 % 16, &PinConfig);
}
_Bool srvWakeupOut(_Bool level)
{
    ...
    if (level)
        ...
        GPIO_pinWrite(22 / 16,
                        1 << (22 % 16),
                        0);
    actual_level = GPIO_pinRead(22 / 16, (22 % 16));
    do { swLogPrintf(UNILOG_PLAT_AP__UNILOG_PLA_DRIVER__srvWakeupOut_1, P_SIG, level, actual_level); {(void)"wake up out %d/%d";} }while(0);
