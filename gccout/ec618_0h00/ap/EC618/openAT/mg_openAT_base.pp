...
#define __CURRENT_FILE_NAME__ mg_openAT_base
#define __EC618 1
#define CHIP_EC618 1
#define CORE_IS_AP 1
...
static void cmd_MINFO_process(void *argument)
{
    ...
    nRet = MG_SYSTEM_GetModuleInfo(nModuleInfo, 65);
    if (nRet < 0)
    {
        mg_openAT_cmdRespError(atHandle, 701);
    }
    MG_UTILS_Snprintf((u8 *)rspBuf, 128, "+MINFO: %s", nModuleInfo);
...
CmsRetId openMINFO(const AtCmdInputContext *pAtCmdReq)
{
    ...
    switch (operaType)
    {
        case AT_TEST_REQ:
        {
            MG_UTILS_Snprintf((u8 *)rspBuf, 128, "+MINFO: <type>");
