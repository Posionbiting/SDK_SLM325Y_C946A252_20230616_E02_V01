...
#define __CURRENT_FILE_NAME__ mg_openAT_engine
#define __EC618 1
#define CHIP_EC618 1
#define CORE_IS_AP 1
...
ST_Thread *g_openAtTaskHandle = ((void *)0);
...
CmsRetId mg_openAT_InDataProcess(AtChanEntityP pAtChanEty, UINT8 *pInput, UINT16 length)
{
    ...
    if (rc != CMS_RET_SUCC)
    {
        ...
        g_openAtDataEngine.cb = ((void *)0);
        g_openAtDataEngine.cb_ctx = ((void *)0);
    }
    return rc;
}
CmsRetId mg_openAT_SetLineMode(UINT8 chanId)
...
static void prvThreadOpenAtEntry(void *param)
{
    ...
    for (;;)
    {
        ...
        MG_OS_EventWait(pThread, &event, (0xFFFFFFFFU));
        switch (event.id)
        {
            case (0x00110000 + 1):
            {
...
BOOL mg_openAT_EngineCallback(openAtCallback_t cb, void *cb_ctx)
{
    ...
    event.id = (0x00110000 + 1);
    event.param1 = (u32)cb;
    event.param2 = (u32)cb_ctx;
    bRet = MG_OS_EventSend(pThread, &event, (0xFFFFFFFFU));
    return (bRet) ? (((BOOL)1)) : (((BOOL)0));
}
ST_Thread *mg_openAT_getOpenAtEngine(void)
{
...
void mg_openAt_EngineCreate(void)
{
    g_openAtTaskHandle = MG_OS_ThreadCreate("openAt", (FU_osThreadEntry)prvThreadOpenAtEntry, ((void *)0), Thread_PRIORITY_NORMAL, 1024 * 4, 16);
}
