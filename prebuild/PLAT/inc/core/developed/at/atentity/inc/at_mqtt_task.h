...
//<!--[SLM328Y][TaskID 35147]ldw@2023-05-27 MQTT multi-link connection.
#ifdef  FEATURE_YY_MQTT_AT_ENABLE
#include "ccio_base.h"
#include "ccio_pub.h"
#include "ccio_tfc.h"
#endif
...
#define MQTT_TX_BUF_DEFAULT        (4+128+2048)    /*header+topic+payload*/
//end-->[ODM][TCP][MeigLink12207]zcxz@2023-02-09 Add TCP and MQTT send length to 2048

//<!--[ODM][MQTT][MeigLink12207]zcxz@2023-02-22 Add MQTT receive buffer to 2048
//#define MQTT_RX_BUF_DEFAULT        (4+128+1024)    /*header+topic+payload*/
#define MQTT_RX_BUF_DEFAULT        (4+128+2048)    /*header+topic+payload*/
//end-->[ODM][MQTT][MeigLink12207]zcxz@2023-02-22 Add MQTT receive buffer to 2048
...
typedef struct
{
    ...
    char *ca_key;
    char *host_name;
//<!--[ODM][BugID 31160]hs@2021-12-09 AT+QMTCFG.
#ifdef  FEATURE_YY_MQTT_AT_ENABLE
    int pdpcid;
    int sslctx_idx;
    int recvmode;
...
//end-->
//<!--[SLM328Y][TaskID 35147]ldw@2023-05-27 MQTT multi-link connection.
    int qmqtt_keepalive_retry_time;
    int qmqtt_task_recv_status_flag;
    int qmqtt_task_send_status_flag;
    int qmqtt_sempr_status_flag;
    osThreadId_t qmqtt_recv_task_handle;
    osThreadId_t qmqtt_send_task_handle;
    osMessageQueueId_t qmqtt_send_msg_handle;
    Mutex qmqtt_mutex1;
    int g_qmqtt_client_status_flag;
    CcioTfcPid_t qmqttTfcPid;
    uint8_t qmqtt_SlpHandler;
//end-->
#endif
...
//<!--[ODM][TaskID 19435]husong@2022-01-28 ADD QMTRECV.
#ifdef  FEATURE_YY_MQTT_AT_ENABLE
typedef struct mqtt_ali_client_info
{
    u8_t  store_count;
    u8_t  store_status[5];
    char*  store_msg[5];
    char*  store_topic[5];
    u16_t store_length[5];
    u8_t  store_clientidx[5][1];
    int  store_msgidx[5][1];
