...
//<!--[ODM][TCP][Bug41785]zcxz@2023-02-27 Change +ECSOSDT only to 2048
#define AT_SOC_UL_LENGTH_MAX_2048 2048
//end-->[ODM][TCP][Bug41785]zcxz@2023-02-27 Change +ECSOSDT only to 2048
#define AT_SOC_UL_LENGTH_MIN 1
//<!--[ODM][TCP][Bug42580]hs@2023-03-20 Modify length for sktrecv
#define AT_SOC_DL_LENGTH_MAX 1400 //1358
//end-->
...
//<!--[ODM][TCP][MeigLink12207]zcxz@2023-02-09 Add TCP and MQTT send length to 2048
//#define SUPPORT_MAX_SOCKET_RAW_DATA_LENGTH    1400
#define SUPPORT_MAX_SOCKET_RAW_DATA_LENGTH    2048
//end-->[ODM][TCP][MeigLink12207]zcxz@2023-02-09 Add TCP and MQTT send length to 2048
...
typedef struct AtRefSocketSndInfo_Tag
{
    UINT8   connectId;
    UINT8   source;
...
typedef struct AtRefPassThroughSockInfo_Tag
{
    UINT8  connectId;
    UINT8  source;
    UINT16 reqHander;  /*4 bytes*/

    UINT8  requestId;
    UINT8  reserved1;
    UINT16 offset;      /*4 bytes*/

    UINT16  dataLen;
    UINT16  reserved2;  /*4 bytes*/

    UINT8  *data;     /* use the dynamic malloc , max is 1460 bytes*/
}AtRefPassThroughSockInfo;


/*AT+QISEND*/
typedef struct AtRefSocSendReq_Tag{
    UINT8      connectId;
    UINT8      reqId;
    UINT16     remotePort;

    /*as cms sock task will continue to send the UL pending list when the first pkg sent left,
      here should to use the actual length to indicate this is a send data request or a failed from
      cms socket task request:
      1> sendLen >0  this is occured by AT+QISEND/QISENDHEX/PASSTHROUGH/CMS ULpendling  ,this is a send data request,
         the QI AT entity should contine to send the data by socket
      2> if sendLen == 0, this means that this is a FAIL CNF report by CMS sock task when process UL pending list
         such as Timeout,etc...
    */
    UINT32     sendLen;
    UINT32     reqSource;
